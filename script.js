...
document.addEventListener("DOMContentLoaded", function() {
    ...
    fetch('data/porto_trip.geojson')
        .then(response => response.json())
        .then(data => {
            L.geoJSON(data, {
                pointToLayer: function (feature, latlng) {
                    // Utilise l'icône personnalisée si présente
                    if (feature.properties && feature.properties.icon) {
                        return L.marker(latlng, {
                            icon: L.icon({
                                iconUrl: feature.properties.icon,
                                iconSize: [32, 32], // ajuste la taille si besoin
                                iconAnchor: [16, 32], // ajuste selon l'icône
                                popupAnchor: [0, -32]
                            })
                        });
                    }
                ...
                coordsToLatLng: function(coords) {
                    // Certains exports GeoJSON inversent ou vident les coordonnées
